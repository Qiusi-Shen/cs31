...
int executeCommands(string cmd, char& plotChar, int& mode, int& badPos);
bool hasCorrectSyntax(string cmd, int& pos);
bool executeOneCommand(string cmd, int& pos, int& r, int& c, char& plotChar, int& mode);
...
int executeCommands(string cmd, char& plotChar, int& mode, int& badPos)
{
    ...
    while (pos != cmd.size())
    {
        if (!executeOneCommand(cmd, pos, r, c, plotChar, mode))
        {
            badPos = pos;
...
//  If the command can not be executed (because it would try to plot a point
...
bool executeOneCommand(string cmd, int& pos, int& r, int& c, char& plotChar, int& mode)
{
    ...
    else if (action == 'H'  ||  action == 'V')
    {
        ...
        if (action == 'V')
            dir = VERT;
        if (!plotLine(r, c, dir, distance, plotChar, mode))
        {
            pos = commandStartPos;
            return false;
        }
...
bool plotLine(int r, int c, int dir, int distance, char plotChar, int fgbg)
{
    ...
    {
      case HORIZ:
        if (c+distance < 1  ||  c+distance > NCOLS)
            return false;
        break;
      case VERT:
        if (r+distance < 1  ||  r+distance > NROWS)
            return false;
        break;
      default:
        return false;
